...
hist(rbinom(1000,100,0.01))
#doesn't look normal

hist(rbinom(1000,100,0.8)) 
#looks pretty normal
...
# Q: write code below to run the lm model and look at the diagnostics, including a histogram of the residua
autoplot(mod)
mod <- lm(fitness ~ body.size,data=soay)
...
ggplot(data = titanic,aes(x=Pclass,y=Survived, color=Sex))+geom_jitter()
