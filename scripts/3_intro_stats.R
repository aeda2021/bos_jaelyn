...
hist(example_data)
qqnorm(example_data)
#hist(example_data,breaks=14)

# Q: after running the above code a bunch of times, what do you conclude about mtcars?
# A: the weight data seems to be within the range of variation for a sample of size n=32 
...
# A: weight is significantly negatively correlated with gas mileage.
...
# A: correlation only answers whether the data covary, but does not give us a model to predict
...
# A: yes, the data are tidy. Rows are car models (observations), and columns are variables, such as gas mileage and weight
...
# A: the slope looks close to -6
# Q: the intercept?
# A: the intercept should be around 35
# Q: the df for the error?  Why?
# A: the degrees of freedom is 30. N is 32, k (number of variables) is 1
...
# A: R's lm() function outputs a model object. We can use additional functions to get detailed information from the model object. 
...
# A: 
# For the residuals vs fitted plot, we want to see residuals more or less normally distributed around dotted line. 
# Our residuals seem mostly normally distributed around the dotted line, though the small sample size makes it hard to tell
# For the Q/Q plot, we want to see residuals along the straight dotted line
# Our residuals follow the Q/Q plot fairly well
# For the scale-location plot, we want to avoid change in the vertical spread of the residuals as we move along the x axis
# This seems fine
# For the residuals vs leverage plot, we want to avoid points in the top right or bottom right corners
# There is one questionable residual here, but really one one
...
# A: the slope and intercept are close to what I estimated. They would probably be closer had I done a better job estimating. 
...
# A: We can re-use the same code even if we change something in the original model. Also, naming the variables minimized the possibility of typos changing results. 
...
# A: yes, this is fine. The confidence interval is for the *mean* of y at each value of x, not the spread of the data
...
aug2 <- augment(car_mod, interval = 'prediction')
  
ggplot(aug2, aes(x = wt, y = mpg)) + 
  geom_point() + 
  geom_line(aes(y = .fitted)) +  # augment uses .whatever for all the new columns it creates to avoid confusion with existing variables
...
# A: for every roughly 1 ton increase in car weight, there is a corresponding approximately 5.3 mpg loss in fuel efficiency
...
# A: ANOVA assumes that the groups have similar variance, and normal distributions. Based on the boxplots, these assumptions seem plausible. 

# Q: what is your guess about the answer to our anova question? be as specific as possible, eg, rank species in order
# A: I expect that mean sepal width does differ significantly between groups
...
# A: they are probably close enough to meeting the assumptions

# fourth, interpret results
summary(iris_mod)
# Q: what does the estimate for the intercept mean? 
# A: the estimate for the intercept is the mean for the first group, in this case setosa
# Q: what do the t values mean, for both the estimate (reference level) and the other comparisons? 
# A: the t value here is the estimate divided by the standard error. For the reference level, this represents
# the number of standard errors the mean is from zero, making it a meaningless statistic. For the other groups,
# the t value is the number of standard errors the group mean is from the reference mean. 

# Q: what does the F and its p value tell you?
# A: the F statistic is the total variation divided by the in group variation. A higher F 
# value suggests a higher likelihood that the samples are drawn from different distributions. 
# The pa values gives the significance of the test. Here, a very low P value indicates a high 
# probability that not all samples are drawn from the same distribution.

anova(iris_mod) # remember this is how we get anova-type results out of the model we ran
# Q: why are the df what they are? 
# A: the degrees of freedom for the species is equal to the number of variables minus 1
# The degrees of freedom for the residuals should equal n - (number of parameters) - 1
# Here, n is 150 and the number of parameters is 2. Therefore, the degrees of freedom is equal to 147.

# Q: what do the two MS values tell you?
# A:Mean Sq Species is the amount of variance between groups; ie the sum of each data point - grand mean
# Mean Sq Residuals is the amount of variance within groups
...
# A: Each pair of means is significantly different. 
# Q: write code below to check your Tukey results (ie, the 'diff' column), against the original data
means <- summarize(group_by(iris,Species),Mean.Sepal.Width = mean(Sepal.Width))
diffs <- c((w[3,2]-w[2,2]),(w[1,2]-w[2,2]),(w[1,2]-w[3,2]))
...
# A:  everything in the top table changes. The residual standard error, R squared,F statistics, and p value 
# stay the same. This is because everything in the top table depends on pairwise differences but the summary 
# statistics, such as F and R^2, are valid for the model as a whole. 
...
# A: the graphical results at the end show confidence intervals around the mean, rather than the full range of the data. 
...
flowers$size <- as.factor(flowers$size)
flowers$colour <- as.factor(flowers$colour)
...
# A: for the colour variable, "orange" will be reference.
# For the size, variable, "large" will be reference
...
# A: The groups seem to have similar variance
...
# A: I think both color and size will predict insect visits individually, but the color x size interaction will be the strongest predictor
...
# A: correlated variables are related to each other, specifically in that they covary either positively or negatively
# variables that have an interaction have a different effect on some dependent variable than either one would singly
...
# A: the null hypothesis isthat all populations have equal variance. Since we cannot reject the null, we can assume that 
# the variances are similar enough to run ANOVA
...
# A: Since these sample sizes are very small, histograms are not terribly useful. 
...
# A: my prediction was fairly accurate
...
# A: this plot includes bars for the standard error

# Q: state your conclusion from this analysis 
# A: mean insect visits differ significantly with both flower color and flower size. The variables (color and size) also have a siginficant interaction effect
...
# A: The histogram shows residuals that seem somewhat right skewed, but maybe not enough to be problematic
# The residuals are (maybe) not normally distributed against the fitted values. This might be an issue.
# The Q/Q plot looks okay. Maybe slightly right skewed.
# The scale-location plot looks fine
# The residuals vs leverage plot looks fine
...
# A: weight and horsepower are both significant predictors of fuel efficiency. For every 1 ton increase in weight, fuel efficience decreases by 3.88 mpg while holding other variables constant
#  for every one unit (horse?) increase in horsepower, fuel efficiency declines by 0.03 mpg while holding other variables constant
...
# A: I predict that mean number of eggs varies significantly with both density and season
...
# A: estimated slope spring: -3/80 eggs/limpet density
#  estimated slope summer: -3/80 eggs/limpet density

# Q: do you think the effect of density depends on season? that is, do you predict an interaction? explain why or why not
# A: I think there is no interaction. The slopes for each season look very similar

# Q: estimate the intercept(s).  can the intercept be meaningfully interpreted in this model?
# A: estimated intercept spring: 2.5 eggs
# estimated intercept summer: 1.7 eggs
# The intercept here corresponds roughly to the predicted eggs produced by one limpet alone in its habitat
...
# A: in that case, R would only consider the interaction effect, not the individual effects of density and season
...
# A: ANCOVA assumes that the variables are independent,  normally distributed within each group, and have approximately equal variance
# it also assumes that the variables are related linearly (if they are related at all)
# and the variance of the independent variable(s) is constant on the dependent variable. 
# In the residuals vs. fitted plot we are checking for the presence of a non-linear relationships. Since we don't see any sort of non-linear pattern 
# in the residuals, this should be fine
# In the Q-Q plot we are checking to see if the data are normally distributed. This data seems approximately normally distributed.
# In the scale-location plot, we check to see whether variance changes along the fitted values. Here it does not.
...
# A: eggs = -0.033650*density + 2.664166
...
# A: eggs = -0.033650*density + 2.664166 + 1.851884
...
# A: for both lines, the slope oe of the line is defined as the estimated density coefficient. 
# For the spring (green) line, the intercept is defined as the estimated intercept. For the summer (orange) line, the interecpt is defined
# as the sum of the estimated intercept and the "SEASONsummer" estimate
...
# A: Limpet egg production varies linearly with density in both the spring and summer seasons, and egg production is significantly higher in the spring than in the summer.
# Density has the same relative effect on egg production regardless of season. 
...
# A: the intercept will change but the slope, p value, and R2 will not. 
...
# A: z scores are unitless, meaning they can be easily compared between datasets
...
# A: the intercept and slope will change, but the p value and R2 will not

mtcars$hp_standardized = scale(mtcars$hp, center=TRUE, scale=TRUE) 
car_mod5 = lm(mpg ~ hp_standardized, data = mtcars)
summary(car_mod5)
# Q: were you right in your predictions above?
# A: I was wrong, the intercept did not change
...
# A: here the intercept is (approximately) zero. Since the p value of the intercept is the significance of the difference between the intercept and 0, the p value becomes 1
...
car_mod6 = lm(mpg_z ~ hp_z + wt_z, data = mtcars)
summary(car_mod6)
...
# A: if both independent and dependent variables are standardized as z scores, the slope will not change.
...
# A: 5 values will take (on average) fewer tries to get an r >= 0.60, though time to p <= 0.05 will be similar to the original
# 100 values will take more time to get to r >= 0.60
# Q: try it and see. suggestion:  don't spend too long on the n=100 version

i <- 1
r <- 0 
while (r < 0.6){
  unif_x <- runif(5, 0, 100)
  unif_y <- runif(5, 0, 100)
  r <-cor(unif_x, unif_y)
  i <- i +1
  print(i)
}

#n = 5 took 12 tries to get r>0.6
...
# n = 100 took more than 100,000 tries to get r>0.6; after that point I interrupted the loop
...
while (p > 0.05){
  unif_x <- runif(5, 0, 100)
  unif_y <- runif(5, 0, 100)
  r <-cor.test(unif_x, unif_y)$p.value
  i <- i +1
  print(i)
}

#interrupted at 100,000
...
while (p > 0.05){
  unif_x <- runif(100, 0, 100)
  unif_y <- runif(100, 0, 100)
  p <-cor.test(unif_x, unif_y)$p.value
  i <- i +1
  print(i)
}
# took 34 loops to get p < 0.05
...
# A: a smaller sample size increases the odds of getting a large effect size by random chance, but does not increase the odds of getting a 
# significant p value by random chance
...
means <- function(sample_size, number_samples) {            # defines the function 'means' with arguments sample_size (ie number of random draws per sample) and number_samples (this will be the number of means plotted in the end)
  ...
  for(i in 1:number_samples) {                       # do the stuff between brackets once for each sample
    g <- runif(sample_size)                            # randomly draw n values from a normal distribution and call this set of numbers 'g'
...
# A: the distribution of means has a much narrower spread than the distribution of data
# Q: was this what you expected? why or why not?
# A: yes, this is equivalent to the observation that standard error of the mean will always be much smaller than the standard deviation
# Q: what do you predict will be the effect of increasing sample_size?  ie, how will the histogram change
# A: increasing the sample size will make the distribution of means even narrower
# Q: what do you predict will be the effect of increasing number_samples?  ie, how will the histogram change
# A: increasing the number of samples will slightly increase the range of the distribution, but not change its overall shape
...
# A: yes, my predictions were correct
...
# A: sample mean +/- sample sd/sqrt(sample size)


# part two, on the central limit theorem
# Q: what is the CLT?  that is, what does it say
# A: the means of a group of identically distributed (but not necessarily normal) random variables will tend towards a normal distribution
# Q: what do you predict will happen when we change the distribution to a uniform one? 
# A: the distribution of means will be normal (wider than the distribution of means from the normal distributions)
...
means <- function(sample_size, number_samples) {           
  ...
  for(i in 1:number_samples) {                       
    g <- runif(sample_size,min=-5,max=5)                           
    output[[i]] <- (mean(g))                          
  }
  hist(output, breaks = 30, xlim = c(-3,3), main = 'distribution of means')
}
