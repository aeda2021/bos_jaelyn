---
title: "AEDA - Bayesian Modeling Bos"
output:
  pdf_document: default
  html_notebook: default
date: 'Last updated: March 11, 2021'
---
...
#[b0,b1,sigma_sq|yi] is proportionate to the product of norm(yi|b0+b1xi,sigma_sq)+norm(x|0,100)+norm(b0|0,100)+norm(b1|0,100)+gamma(sigma_sq|1,0.1)
...
*Answer*: The marginal probability of a variable is the probability distribution of that variable summed across the distributions of all of the other parameters.
...
lmm.data <- read.csv("C:/github/aeda_bos/raw-data/mixedEffectDat.csv")
```
...
```{r Try it - data}
crabs <- read.csv('C:/github/aeda_bos/raw-data/crab_burrows.csv')
colnames(crabs) <-c("burrows","salinity")
...
datList_crabs <- list(N=nrow(crabs),y=crabs$burrows,x=crabs$salinity)

initial_values_abund <- replicate(3, list(list(
  b0 = rnorm(1,0, 10), 
  b1 = rnorm(1,0, 10)
)))
```
...
fm_crabs_mcmc <- as.mcmc(fm_crabs) 
# Then a data frame,
...
fm_crabs_dat <- as.data.frame(as.matrix(fm_crabs_mcmc))
```

Run diagnostics - make traceplots and calculate the Gelman diagnostic.
```{r Crab model - diagnostics, eval=F}
traceplot(fm_crabs_mcmc)
gelman.diag(fm_crabs_mcmc)
```
...
plotPost(exp(fm_crabs_dat$b0), main = "intercept")
plotPost(exp(fm_crabs_dat$b1), main = "slope salinity x burrows",xlim=c(1.0,1.1))
plotPost(exp(fm_crabs_dat$b1*rep(mean(crabs$salinity),length(fm_crabs_dat$b0)) + fm_crabs_dat$b0),main="mean # of burrows posterior")

summary(fm_crabs)
summary(fm_crabs_mcmc)

#Posterior means

mean_burrows <- exp(fm_crabs_dat$b1*rep(mean(crabs$salinity),length(fm_crabs_dat$b0)) + fm_crabs_dat$b0)
...
*Answer:*
* The expected number of burrows from the model is 10.9 with a 95% confidence interval of 9.58 to 12.6.
* The intercept is the expected value of the number of burrows when salinity equals zero.
Since we centered the salinity measurements at zero, this is equal to the overall posterior expected number of burrows. If we hadn't centered salinity at zero, the intercept would not be the same as the expected number of burrows. 
*On the scale of the data, the estimated slope for the relationship between salinity and the number of burrows is 1.03, with a 95% confidence interval of 1.02-1.05
* The probably that salinity has no effect is less than  0.025
...
ticks <- read.csv("C:/github/aeda_bos/raw-data/Ehrlichia_occurrence.csv")
glimpse(ticks)
ticks$site
```
...
datList.ticks <- list(
  ...
  x3 = ticks$evergreen,
  # site
  site = as.numeric(factor(ticks$site)),
  # number of sites
...
params <- c('alpha0', 'b1', 'b2', 'b3','sig.site')
...
fm_ticks_mcmc <- as.mcmc(fm.ticks)
fm_ticks_dat <- as.data.frame(as.matrix(fm.ticks.mcmc))
```

Run traceplots and Gelman's diagnostic:
```{r Try it- diagnostics, eval=F}
traceplot(fm_ticks_mcmc)
gelman.diag(fm_ticks_mcmc)
...
plotPost(plogis(fm_ticks_dat$alpha0),main="grand mean")
plotPost(plogis(fm_ticks_dat$b1),main="slope forest x ticks")
plotPost(plogis(fm_ticks_dat$b2),main="slope edge x ticks")
plotPost(plogis(fm_ticks_dat$b3),main="slope evergreen x ticks")

summary(fm.ticks)
summary(fm_ticks_mcmc)
...
*Answer:*
The overall EC occupancy was 0.265. All of the variables have positive relationships with tick occurance. In general, I would conclude that  proportion of forest within the surrounding 100 ha, length of forest edge within that 100 ha, and the proportion of that forest that is evergreen/coniferous all increase the probability of finding ticks in a given plot. 
